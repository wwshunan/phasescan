...
class MyCustomToolbar(NavigationToolbar):
    ON_CUSTOM_UP = wx.NewIdRef(count=1)
    ON_CUSTOM_DOWN = wx.NewIdRef(count=1)
    ON_CUSTOM_REMOVE = wx.NewIdRef(count=1)
    ON_CUSTOM_FIT = wx.NewIdRef(count=1)
...
class FitThread(CAThread):
    ...
    def clear_up(self, rf_phase, w_in, w_gain, amp, x_plot, y_plot):
        ...
        wx.CallAfter(self.master.display.write_line,
                     '{0}\nPhase: {1:.1f}\nIn Energy: {2:.3f}\nOut Energy: {3:.3f}\nEpeak: {4:.2f}\n'.format(
                         cavity_name, rf_phase, w_in-w_gain, w_in, amp))
        wx.CallAfter(self.master.updateGraph, self.master.fit_line, x_plot, y_plot)
        wx.CallAfter(self.master.update_next_cav_energy, w_in)
        wx.CallAfter(self.master.setPhase.SetValue, str(round(rf_phase, 2)))
        if log_excel:
            data = get_data(log_excel)
            data['Sheet 1'].append([cavity_name, round(float(rf_phase), 1),
...
class WorkThread(CAThread):
    ...
    def scan(self, cavity_name, cavity_pv_set, cavity_pv_readback, bpm_pv_name, cavity_ready):
        ...
        while ((self.stop_phase - start_phase) * self.phase_step > 0):
            ...
            f.write('%s\t' % np.array2string(average)[1:-1])
            f.write('%s\n' % np.array2string(rms_err)[1:-1])
    ...
    def shift_bpm_phases(self, xdata, ydata):
        smooth_start_idx = 0
        for i in range(xdata.shape[0]):
            if abs((xdata[i+1] - xdata[i]) - self.phase_step) < 0.1:
                smooth_start_idx = i
                break

        i = smooth_start_idx
        while i < xdata.shape[0] - 1:
            shift_min, shift_max = -3, 3
            grads = []
            for j in range(shift_min, shift_max+1):
                grad = (ydata[i+1]-ydata[i]+j*360) / (xdata[i+1]-xdata[i])
                grads.append(grad)
            if i == smooth_start_idx:
                min_idx = np.argmin([abs(grad) for grad in grads])
            else:
                min_idx = np.argmin([abs(grad-prev_grad) for grad in grads])
                #if abs((grads[min_idx] - prev_grad)) > 0.8:
                #    xdata = np.delete(xdata, i+1)
                #    ydata = np.delete(ydata, i+1)
                #    print(xdata, ydata)
                #    time.sleep(0.2)
                #    continue
    ...
    def smooth_data(self, xdata, ydata, errors):
        ...
        self.shift_bpm_phases(xdata, ydata[:, 0])
        self.shift_bpm_phases(xdata, ydata[:, 1])
        #xdata_bpm1, ydata_bpm1 = self.shift_bpm_phases(xdata, ydata[:, 0])
        #xdata_bpm2, ydata_bpm2 = self.shift_bpm_phases(xdata, ydata[:, 1])
        #intersect_data, reserved_idx_bpm1, reserved_idx_bpm2 = \
        #    np.intersect1d(xdata_bpm1, xdata_bpm2, return_indices=True)
        #reserved_idx = np.in1d(xdata, intersect_data)
        #xdata = xdata[reserved_idx]
        #ydata_bpm1 = ydata_bpm1[reserved_idx_bpm1]
        #ydata_bpm2 = ydata_bpm2[reserved_idx_bpm2]
        #ydata = np.c_[ydata_bpm1, ydata_bpm2]
        #errors = errors[reserved_idx]
...
class MyFrame(wx.Frame):
    ...
    #cavity_ready += ['HWR:LLRF:2-4:phaseloopstatus', 'HWR:LLRF:2-5:phaseloopstatus', 'SCRF:CAV12:READY']
    #cavity_ready += ['HWR:LLRF:3-1:phaseloopstatus', 'SCRF:CAV14:READY', 'SCRF:CAV15:READY']
    ...
    #           ('Bpm:19-P11', 'Bpm:20-P11'), ('Bpm:20-P11', 'Bpm:21-P11'), ('Bpm:21-P11', 'Bpm:22-P11')]
               ('Bpm:21-P11', 'Bpm:22-P11'), ('Bpm:21-P11', 'Bpm:22-P11'), ('Bpm:21-P11', 'Bpm:22-P11')]
    #bpm_pv += [('Bpm:21-P11', 'Bpm:22-P11'), ('Bpm:22-P11', 'Bpm:23-P11'), ('Bpm:23-P11', 'Bpm:24-P11'),
    bpm_pv += [('Bpm:21-P11', 'Bpm:22-P11'), ('Bpm:24-P11', 'Bpm:25-P11'), ('Bpm:24-P11', 'Bpm:25-P11'),
    ...
    distance_cav_bpm += [0.6395, 0.6395, 0.6438, 0.7856, 0.7856, 0.7856]
    #distance_cav_bpm += [0.6395, 0.6395, 0.6438, 0.6419, 1.9357, 0.7856]    cm3 before change
    #distance_cav_bpm += [0.7856, 0.7904, 0.7835, 1.5046, 1.86044]       cm4 before change
    distance_cav_bpm += [0.7856, 1.5046, 1.5046, 1.5046, 1.86044]
    ...
    def __init__(self):
        wx.Frame.__init__(self, None, -1, "PhaseScan double bpm")
    ...
    def setCavPhase(self, event):
        ...
        rb_phase_value = phase_get_pv.get()
        self.rbPhase.SetLabel('{0:.2f}'.format(rb_phase_value))
        self.display.write_line('{0} phase has been set'.format(self.cavity.GetValue()))

        #self.getEnergyFromTOF()
    ...
    def getEnergyFromTOF(self):
        average_loop = 6
        phase_tolerance = 10
        time_step = 1
        if self.next_energy != 0:
            bpm1_pv_name, bpm2_pv_name = self.bpm_pv[self.cavity_id]
            bpm1_pv = PV(bpm1_pv_name)
            bpm2_pv = PV(bpm2_pv_name)
            bpm1_vals = []
            bpm2_vals = []

            for i in range(average_loop):
                bpm1_vals.append(bpm1_pv.get())
                bpm2_vals.append(bpm2_pv.get())
                time.sleep(time_step)

            bpm1_std = np.std(bpm1_vals)
            bpm2_std = np.std(bpm2_vals)
            if (bpm1_std < phase_tolerance) or (bpm2_std < phase_tolerance):
                bpm1_val = np.average(bpm1_vals)
                bpm2_val= np.average(bpm2_vals)
                bpm_diff = bpm1_val - bpm2_val

                type_name = self.type.GetValue()
                mass = self.particle_mass_charge[type_name]['mass']
                freq = 162.5e6
                cavity_name = self.cavityList[self.cavity_id]
                tof_tool = TofPhaseTransformer(cavity_name, freq, mass)
                periods = tof_tool.get_tof_periods(bpm_diff, self.next_energy)
                self.next_energy = tof_tool.phase_to_energy(periods, bpm_diff)
    ...
    def clear_graph(self):
        lines_clear = ['first_bpm_line', 'second_bpm_line', 'bpm_diff_line', 'selected', 'fit_line']
        lines_exists = [hasattr(self, line_exist) for line_exist in lines_clear]
        if all(lines_exists):
