...
class DataFitBase(object):
    def __init__(self, **kwargs):
        self.cav_phases_degree = kwargs['cavity_phase']
        ...
        self.epk_factor = kwargs['epk_factor']
        self.cavity_polarity = kwargs['cavity_polarity']
...
class DoubleBPMFit(DataFitBase):
    ...
    def bpm_fit(self):
        ...
        computed_bpm_diff = [self.get_bpm_diff(field_factor, phase_in + delta)
                               for delta in self.cav_phases_rad]
        ...
        rf_phase = phase_wrapping(rf_phase)
        return (rf_phase, exit_energy, field_factor * self.epk_factor, error,
                self.cav_phases_degree, computed_bpm_diff + plsq[0][2])

    def get_bpm_diff(self, field_factor, phase_in):
        W = self.Win
        t = 0
        a = 0
        b = 0
        for i in range(len(self.Ez) - 1):
            ...
            # b += c * Ez[i] * cos(phi) * dz
            t += self.dz / (0.5 * (beta + betaExit) * c)
        tof = self.distance / (betaExit * c)
        # traceWin_phi = actan(a / b)
        return -(self.freq * tof) * 180 * 2 * self.bpm_harm * self.bpm_polarity

    def residuals(self, p):
        field_factor, phase_in, offset = p
        bpm_diff = self.bpm_phases[:, 1] - self.bpm_phases[:, 0]
        err = bpm_diff - [
            self.get_bpm_diff(field_factor, phase_in + delta) + offset
            for delta in self.cav_phases_rad]
        return err
